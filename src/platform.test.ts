...
test("platform adapter resolves Windows directory fallbacks from home", () => {
  const dirs = getPlatformDirectories({
    platform: "win32",
    env: {},
    homeDir: windowsHome,
  });
...
test("generic shell and path-length helpers stay centralized", () => {
  assert.equal(
    getShellForPlatform({ platform: "linux", env: { SHELL: "/bin/bash" } }),
    "/bin/bash",
  );
...
test("executable permission handling is platform-specific", () => {
  ...
  assert.equal(
    getPlatformAdapter("win32").hasExecutablePermission(
  ...
  assert.equal(
    mac.hasExecutablePermission("/Applications/GOAT/goat-engine", fileSystem),
    false,
  );
...
test("atomic replacement writes a complete replacement in place", async () => {
  ...
  await getPlatformAdapter("darwin").replaceFileAtomically(target, "new", {
    tempSuffix: "test",
  });
