...
export async function createWindowsJobContainment(options: {
  ...
  // A guard that dies mid-protocol surfaces as an async stream error; swallow
  // it here so write callbacks and protocol timeouts stay the only failure
  // paths instead of crashing on an unhandled 'error' event.
