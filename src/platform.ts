...
export type CredentialStorageKind = 'windows-credential-manager' | 'macos-keychain';
...
export interface PlatformAdapter {
  ...
  hasExecutablePermission(path: string, fileSystem?: ExecutablePermissionFileSystem): boolean;
  ensureExecutablePermission(path: string, fileSystem?: ExecutablePermissionFileSystem): void;
  terminateProcess(
    child: Pick<ChildProcess, 'kill' | 'pid'>,
    signal: NodeJS.Signals,
    options?: { runCommand?: ProcessTerminatorCommand },
  ): void;
  replaceFileAtomically(path: string, data: string | Uint8Array, options?: AtomicReplaceOptions): Promise<void>;
}
...
export function getPlatformAdapterForPlatform(platform: NodeJS.Platform): PlatformAdapter | null {
  const supported = getSupportedPlatform(platform);
  return supported ? getPlatformAdapter(supported) : null;
}

export function getPlatformDirectories(options: PlatformDirectoryOptions = {}): PlatformDirectories {
...
export function getShellForPlatform(options: PlatformDirectoryOptions = {}): string {
  const platform = options.platform ?? getRuntimePlatform();
  const adapter = getPlatformAdapterForPlatform(platform);
...
export function getForwardedSignals(platform: NodeJS.Platform): NodeJS.Signals[] {
  return getPlatformAdapterForPlatform(platform)?.getForwardedSignals() ?? ['SIGINT', 'SIGTERM', 'SIGHUP'];
}

export function getParentExitSignal(platform: NodeJS.Platform): NodeJS.Signals {
  return getPlatformAdapterForPlatform(platform)?.getParentExitSignal() ?? 'SIGHUP';
}

export function hasPathLengthProblem(filePath: string, platform: NodeJS.Platform = getRuntimePlatform()): boolean {
  return getPlatformAdapterForPlatform(platform)?.hasPathLengthProblem(filePath) ?? false;
}

export async function replaceFileAtomically(
  filePath: string,
  data: string | Uint8Array,
  options: AtomicReplaceOptions & { pathModule?: typeof path.win32 | typeof path.posix } = {},
): Promise<void> {
  ...
  try {
    await fileSystem.writeFile(tempPath, data, options.mode === undefined ? undefined : { mode: options.mode });
...
function windowsDirectories(options: PlatformDirectoryOptions = {}): PlatformDirectories {
  const env = options.env ?? process.env;
  const home = options.homeDir ?? os.homedir();
...
function macosDirectories(options: PlatformDirectoryOptions = {}): PlatformDirectories {
  const home = options.homeDir ?? os.homedir();
  return {
    appData: path.posix.join(home, 'Library', 'Application Support', 'goat'),
    config: path.posix.join(home, 'Library', 'Application Support', 'goat', 'config'),
...
function ensureExecutablePermission(
  ...
  if (platform === 'win32' || hasExecutablePermission(platform, filePath, fileSystem)) return;
...
function runProcessTerminator(command: string, args: readonly string[]): ProcessTerminatorResult {
...
const windowsAdapter: PlatformAdapter = {
  ...
  hasExecutablePermission: (filePath, fileSystem) => hasExecutablePermission('win32', filePath, fileSystem),
  ensureExecutablePermission: (filePath, fileSystem) => ensureExecutablePermission('win32', filePath, fileSystem),
  terminateProcess: terminateWindowsProcessTree,
  replaceFileAtomically: (filePath, data, options = {}) =>
    replaceFileAtomically(filePath, data, { ...options, pathModule: path.win32 }),
};
...
const macosAdapter: PlatformAdapter = {
  ...
  hasExecutablePermission: (filePath, fileSystem) => hasExecutablePermission('darwin', filePath, fileSystem),
  ensureExecutablePermission: (filePath, fileSystem) => ensureExecutablePermission('darwin', filePath, fileSystem),
  terminateProcess: (child, signal) => terminateWithSignal(child, signal),
  replaceFileAtomically: (filePath, data, options = {}) =>
    replaceFileAtomically(filePath, data, { ...options, pathModule: path.posix }),
};
