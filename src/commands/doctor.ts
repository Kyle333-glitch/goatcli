...
export async function runDoctor(options: DoctorOptions = {}): Promise<void> {
  ...
  const engineContractVersion =
    options.engineContractVersion ?? getEngineContractVersion();
  ...
  if (engineResolution.path) {
    pathsToCheck.push({
      name: "Engine Executable Path",
      path: engineResolution.path,
    });
  }
  if (engineResolution.manifestPath) {
    pathsToCheck.push({
      name: "Engine Manifest Path",
      path: engineResolution.manifestPath,
    });
  }
  ...
  try {
    validatedEngine = validateEngine(
      toResolvedEngine(engineResolution),
  ...
  const manifestStatusText = engineResolution.manifestPath
    ...
    : engineResolution.developmentOverride
      ? "not required for explicit development injection"
      : "not resolved";
  const sourceText =
  ...
  if (!gitVersion) {
    errors.push(
      "Git is not installed or not available in the system PATH. Please install Git.",
    );
  }

  if (!appDataWritable.writable) {
    errors.push(
      `Application data directory is not writable: ${appDataWritable.error || "Permission denied"}`,
    );
  }
  if (!configWritable.writable) {
    errors.push(
      `Configuration directory is not writable: ${configWritable.error || "Permission denied"}`,
    );
  }
  if (!cacheWritable.writable) {
    errors.push(
      `Cache directory is not writable: ${cacheWritable.error || "Permission denied"}`,
    );
  }
  ...
  for (const status of pathLengthStatuses) {
    if (status.hasProblem) {
      warnings.push(
        `Path is too long on Windows and may cause issues: ${status.name} (${status.path})`,
      );
    }
  }
  ...
  if (legacyConfigExists) {
    warnings.push(
      `Legacy ${legacyConfigPath} was found. goatcli ignores config.json enginePath; use the local engine install path.`,
    );
  }

  if (engineValidationError) {
    errors.push(
      formatEngineContractError(engineValidationError).replace("\n", " "),
    );
  }
  ...
  if (platform?.platform === "win32") {
    if (longPathsEnabled === false) {
      warnings.push(
        "Windows Registry LongPathsEnabled is disabled. Path operations longer than 260 characters might fail.",
      );
    }
    if (windowsIssues) {
      if (!windowsIssues.pathExtHasExe) {
        errors.push(
          "PATHEXT environment variable does not contain .EXE. Windows might fail to launch executables.",
        );
      }
      if (!windowsIssues.npmBinInPath) {
        warnings.push(
          "npm global binaries directory not detected in PATH. You may not be able to run goat globally.",
        );
      }
    }
  }
  ...
  if (platform?.platform === "darwin" && engineResolution.path && macStatus) {
    if (macStatus.isQuarantined) {
      warnings.push(
        `Engine binary is quarantined by macOS Gatekeeper. Run: xattr -d com.apple.quarantine "${engineResolution.path}"`,
      );
    }
    if (macStatus.exists && !macStatus.codeSignValid) {
      warnings.push(
        "Engine binary signature verification failed. The binary may be unsigned or modified.",
      );
    }
  }
  ...
  if (errors.length > 0 || warnings.length > 0) {
    ...
    outro(
      "GOAT Doctor finished with warnings or errors. See recommendations above.",
    );
    return;
  }
